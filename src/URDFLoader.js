...
export default class URDFLoader {
    ...
    parse(content, workingPath = this.workingPath) {
        ...
        function resolvePath(path) {
            ...
            if (typeof packages === "string") {
                ...
                if (targetPkg in packages) {
                    ...
                    console.error(
                        `URDFLoader : ${targetPkg} not found in provided package list.`
                    );
                    return null;
                }
            }
        ...
        function processUrdf(data) {
            ...
            const robotNode = children
                .filter((c) => c.nodeName === "robot")
                .pop();
        ...
        function processRobot(robot) {
            ...
            const links = robotNodes.filter(
                (c) => c.nodeName.toLowerCase() === "link"
            );
            const joints = robotNodes.filter(
                (c) => c.nodeName.toLowerCase() === "joint"
            );
            const materials = robotNodes.filter(
                (c) => c.nodeName.toLowerCase() === "material"
            );
            ...
            links.forEach((l) => {
                ...
                const isRoot =
                    robot.querySelector(`child[link="${name}"]`) === null;
                linkMap[name] = processLink(
                    l,
                    visualMap,
                    colliderMap,
                    isRoot ? obj : null
        ...
        function processJoint(joint) {
            ...
            children.forEach((n) => {
                ...
                if (type === "origin") {
                    ...
                    obj.limit.lower = parseFloat(
                        n.getAttribute("lower") || obj.limit.lower
                    );
                    obj.limit.upper = parseFloat(
                        n.getAttribute("upper") || obj.limit.upper
                    );
                }
            });
            ...
            const axisNode = children.filter(
                (n) => n.nodeName.toLowerCase() === "axis"
            )[0];

            if (axisNode) {
                const axisXYZ = axisNode
                    .getAttribute("xyz")
                    .split(/\s+/g)
                    .map((num) => parseFloat(num));
                obj.axis = new THREE.Vector3(
                    axisXYZ[0],
                    axisXYZ[1],
                    axisXYZ[2]
                );
        ...
        function processLink(link, visualMap, colliderMap, target = null) {
            ...
            if (parseVisual) {
                const visualNodes = children.filter(
                    (n) => n.nodeName.toLowerCase() === "visual"
                );
            ...
            if (parseCollision) {
                const collisionNodes = children.filter(
                    (n) => n.nodeName.toLowerCase() === "collision"
                );
        ...
        function processLinkElement(vn, materialMap = {}) {
            ...
            const materialNode = children.filter(
            ...
            children.forEach((n) => {
                ...
                if (type === "geometry") {
                    ...
                    if (geoType === "mesh") {
                        ...
                        if (filePath !== null) {
                            const scaleAttr =
                                n.children[0].getAttribute("scale");
                            if (scaleAttr) {
                        ...
                        primitiveModel.geometry = new THREE.BoxGeometry(
                            1,
                            1,
                            1
                        );
                        ...
                        const size = processTuple(
                            n.children[0].getAttribute("size")
                        );
                        ...
                        const primitiveModel = new THREE.Mesh();
                        primitiveModel.geometry = new THREE.SphereGeometry(
                            1,
                            30,
                            30
                        );
                        ...
                        const radius =
                            parseFloat(n.children[0].getAttribute("radius")) ||
                            0;
                        ...
                        const primitiveModel = new THREE.Mesh();
                        primitiveModel.geometry = new THREE.CylinderGeometry(
                            1,
                            1,
                            1,
                            30
                        ...
                        const radius =
                            ...
                            0;
                        const length =
                            parseFloat(n.children[0].getAttribute("length")) ||
                            0;
