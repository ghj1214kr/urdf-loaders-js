    ...
    document.addEventListener("drop", (e) => {
        ...
        dataTransferToFiles(e.dataTransfer)
            .then((files) => {
                ...
                // of files
                const fileNames = Object.keys(files).map((n) =>
                    cleanFilePath(n)
                );
                viewer.urlModifierFunc = (url) => {
                    // find the matching file given the requested url
                    const cleaned = cleanFilePath(
                        url.replace(viewer.package, "")
                    );
                    const fileName = fileNames
                        .filter((name) => {
                            ...
                            return (
                                cleaned.substr(cleaned.length - len) ===
                                name.substr(name.length - len)
                            );
                        })
                        .pop();
                ...
                const availableModels = fileNames.filter((n) =>
                    /urdf$/i.test(n)
                );
                // remove existing entries from #urdf-options
                const urdfOptionsContainer =
                    document.querySelector("#urdf-options");
                while (urdfOptionsContainer.firstChild) {
                    urdfOptionsContainer.removeChild(
                        urdfOptionsContainer.firstChild
                    );
                }
